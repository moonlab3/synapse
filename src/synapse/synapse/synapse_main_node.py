...
class SynapseMainNode(Node):
    def __init__(self):
        ...
        # self.action_buffer = ActionChunkBuffer()  # legacy — see class docstring above
...
class SynapseMainNode(Node):
    def __init__(self):
        ...
        # Old flat-schema robots (no 'components' key) — each is its own # single-component group, identical to today's behavior.
        for name, cfg in self.robots_cfg.items():
            if name in grouped_component_names:
                continue
            self.groups[name] = {
        ...
        # --- States: still one subscription per GROUP, skip if resolve_states returned None ---
        self.robot_subscribers = {}
        for group_name, group in self.groups.items():
            if group['states'] is None:
                continue
    ...
    def tick(self):
        ...
        if action is not None:
            ...
            if self.inference_future is not None and self.inference_future.done():
                ...
                if new_action_chunk:
                    adapter = self.brain_adapters[self.running_brain]
                    messages = build_target_messages(new_action_chunk, adapter.chunk_dt, self.component_targets)
                    for component_name, msg in messages.items():
                        if component_name not in self.target_publishers:
                            continue
                        self.target_publishers[component_name].publish(msg)
